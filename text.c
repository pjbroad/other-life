...
int filter_or_ignore_text (char *text_to_add, int len, int size, Uint8 channel)
{
	...
	*/
	...
	if (channel == CHAT_SERVER) {
		if (!strncasecmp(text_to_add+1, "Today is a special day:", 23) ||
		    !strncasecmp(text_to_add+1, "Day ends:", 9) ||
		    !strncasecmp(text_to_add+1, "Just an ordinary day", 21)) {
		        is_acid_rain_day = 0;
		}
		if (!strncasecmp(text_to_add+1, "Today is a special day:\nAcid Rain Day", 37)) {
		        is_acid_rain_day = 1;
		}
		else if (!strncasecmp(text_to_add+1, "You started to harvest ", 23)) {
