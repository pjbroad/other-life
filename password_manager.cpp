...
namespace Password_Manaager
{
...
#ifdef OTHER_LIFE
		if ((words.size() == 3) && (words[1].size() < MAX_USERNAME_LENGTH))
#else
	...
	int Window::ui_scale(Logins *logins, window_info *win)
	{
		...
		checkbox_id = checkbox_add_extended(win->window_id,  2, NULL, border_x, height,
			win->box_size, win->box_size, 0, win->current_scale,  newcol_r, newcol_g, newcol_b, &show_passwords);
		checkbox_label_id = label_add_extended(window_id, 3, NULL, 2 * border_x + win->box_size, height, 0, win->current_scale, newcol_r, newcol_g, newcol_b, show_passwords_str);
	...
	void Window::open(void)
	{
		if (window_id < 0)
		{
			...
			scroll_id = vscrollbar_add_extended (window_id, 1, NULL, 0, 0, 0, 0, 0, 1.0, newcol_r, newcol_g, newcol_b, 0, 1, 0);
