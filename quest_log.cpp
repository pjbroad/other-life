...
void Quest_List::open_window(void)
{
	if (win_id < 0)
	{
		...
		scroll_id = vscrollbar_add_extended(win_id, scroll_id, NULL, 0, 0, 0, 0, 0, 1.0, newcol_r, newcol_g, newcol_b, 0, 1, quests.size()-1);
...
void Questlog_Window::open(void)
{
	if (questlog_win < 0)
	{
		...
		quest_scroll_id = vscrollbar_add_extended (questlog_win, quest_scroll_id, NULL, qlwinwidth - win->box_size, win->box_size, win->box_size, qlwinheight - win->box_size, 0, 1.0, newcol_r, newcol_g, newcol_b, last_entry, 1, last_entry);
