...
static void clear_chat_tabs_updated();
...
static void clear_tab_buttons_updated();
...
static void clear_tabs_updated ()
{
...
static chat_channel channels[MAX_CHAT_TABS];
...
static void clear_chat_tabs_updated (void)
{
	int itab;
...
static void create_chat_window(void)
{
	...
	chat_scroll_id = vscrollbar_add_extended (chat_win, chat_scroll_id, NULL, chat_win_width - CHAT_WIN_SCROLL_WIDTH, CLOSE_SIZE, CHAT_WIN_SCROLL_WIDTH, chat_win_height - 2*CLOSE_SIZE, 0, 1.0f, newcol_r, newcol_g, newcol_b, 0, 1, 0);
	widget_set_OnDrag (chat_win, chat_scroll_id, chat_scroll_drag);
	widget_set_OnClick (chat_win, chat_scroll_id, chat_scroll_click);
	
	chat_tabcollection_id = tab_collection_add_extended (chat_win, chat_tabcollection_id, NULL, CHAT_WIN_SPACE, CHAT_WIN_SPACE, inout_width, tabcol_height, 0, DEFAULT_SMALL_RATIO, newcol_r, newcol_g, newcol_b, MAX_CHAT_TABS);
	...
	channels[0].out_id = text_field_add_extended (channels[0].tab_id, channels[0].out_id, NULL, 0, 0, inout_width, output_height, 0, chat_zoom, newcol_r, newcol_g, newcol_b, display_text_buffer, DISPLAY_TEXT_BUFFER_SIZE, FILTER_ALL, CHAT_WIN_SPACE, CHAT_WIN_SPACE);
	...
	if(input_widget == NULL) {
		...
		id = text_field_add_extended (chat_win, 19, NULL, CHAT_WIN_SPACE, input_y, inout_width, input_height, TEXT_FIELD_BORDER|TEXT_FIELD_EDITABLE|TEXT_FIELD_NO_KEYPRESS, chat_zoom, newcol_r, newcol_g, newcol_b, &input_text_line, 1, FILTER_ALL, CHAT_WIN_SPACE, CHAT_WIN_SPACE);
				...
				case CHAT_LIST:
					if(chan_sel_win >= 0) {
						...
						if(chan_name_queue->nodes >= CS_MAX_DISPLAY_CHANS && chan_sel_scroll_id == -1) {
							...
							chan_sel_scroll_id = vscrollbar_add_extended (chan_sel_win, 0, NULL, 0, 0, 0, 0, 0, 1.0, newcol_r, newcol_g, newcol_b, 0, 1, len);
						}
...
static int add_tab_button (Uint8 channel)
{
	...
	tabs[tabs_in_use].button = button_add_extended(tab_bar_win, cur_button_id++, NULL, tab_bar_width, 0, 0, tab_bar_height, BUTTON_SQUARE, tab_bar_zoom, newcol_r, newcol_g, newcol_b, label);
...
static int display_channel_color_win(Uint32 channel_number)
{
	...
	if(channel_color_win < 0) 
	{
		...
		/* Add labels */
		color_label_id = label_add_extended(channel_color_win, color_label_id, NULL, 0, 0, 0, 0.9f, newcol_r, newcol_g, newcol_b, channel_color_str);
		...
		color_set_button_id = button_add_extended(channel_color_win, color_set_button_id, NULL, 0, 0, 0, 0, 0, 1.0, newcol_r, newcol_g, newcol_b, channel_color_add_str);
		widget_set_OnClick(channel_color_win, color_set_button_id, click_channel_color_handler);
		color_delete_button_id = button_add_extended(channel_color_win, color_delete_button_id, NULL, 0, 0, 0, 0, 0, 1.0, newcol_r, newcol_g, newcol_b, channel_color_delete_str);
