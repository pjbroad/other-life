...
int create_buddy_interface_win(const char *title, void *argument)
{
	...
	if(strcmp(title, buddy_add_str) == 0) {
		if(buddy_add_win < 0) {
			...
			x = string_width+5;
			y = 5;
			buddy_name_input_id = pword_field_add_extended(buddy_add_win, buddy_name_input_id, NULL, x, y, buddy_add_x_len-x*2+10, 20, P_TEXT, 0.9f, newcol_r, newcol_g, newcol_b, buddy_name_buffer, MAX_USERNAME_LENGTH);
